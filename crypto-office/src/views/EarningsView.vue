...
let loadEarnings = function () {
  ...
  fetch("http://localhost:8000/earnings", {
    ...
    .then(data => {
      ...
      storeInvestments.getAllEarnings(earningSum);
...
let addItem = function () {
  ...
  fetch("http://localhost:8000/earnings/0", {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
...
let deleteitem = function (id) {
  if (confirm("Do you really want delete this item?")) {
    fetch("http://localhost:8000/earnings/" + id, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json'
      }
    ...
    <div class="container">
      ...
      <div class="table_wrapper">
        <table>
          ...
          <tbody>
            <tr v-for="earningItem in earnings" :key="earningItem.id" class="earnings">
              ...
              <td class="column5">
                <button class="column_delete-btn" v-on:click.prevent="deleteitem(earningItem.id)">
                ...
                <button>
                  <RouterLink class="modify_btn" :to="`/earnings/${earningItem.id}`">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50" width="16px" height="16px">
                      <path
